...
PlayState.create = function () {
    ...
    ////////////////////
    //Score Bar Section
    this.scoreBar = new Kiwi.GameObjects.Sprite(this,this.textures.scoreBar, 0, 400);
    this.addChild(this.scoreBar);

    this.score = 0;
    this.scoreTF = new Kiwi.HUD.Widget.TextField(this.game, "SCORE: " + this.score, 20, 415);
    this.scoreTF.style.color = "#ffffff";
    this.scoreTF.style.fontWeight = "bolder";
    this.scoreTF.style.fontSize = "20px";
    this.scoreTF.style.fontFamily = "Roboto"
    this.scoreTF.style.letterSpacing = "1px"
    this.game.huds.defaultHUD.addWidget(this.scoreTF);

    this.started = false;
    this.animating = false;
    this.numTilesAnimated = 0;
    ...
    */
    ...
    for (var y = 0; y < this.height; y++) {
        for (var x = 0; x < this.width; x++) {
            ...
            var tempCreate = new Kiwi.GameObjects.Sprite(PlayState, PlayState.textures.fruits, x * PlayState.tileSize, y * PlayState.tileSize);
...
PlayState.update = function(){
    ...
    /**//////////////////**/
    this.scoreTF.text = "Your score: " + this.score;
...
PlayState.clickTile = function (mouseX, mouseY) {
    ...
    if (this.curr == '') {
        ...
        this.curr = tile;
        this.curr.scaleX = 1.2;
        this.curr.scaleY = 1.2;
    } else if (this.curr == tile) {
        //Deselect current tile
        this.curr.scaleX = 1.0;
        this.curr.scaleY = 1.0;
        this.curr = '';
    } else {
        this.curr.scaleX = 1.0;
        this.curr.scaleY = 1.0;
        ...
        for(var j=this.height-1; j>=0 ;j--){
            ...
            if(!tile.visible){
                ...
                if (lowestVisible == -1) { // No visible tile above. Therefore randomize new one.
                    ...
                    newCount++;
                    var rand = Math.floor(Math.random() * PlayState.gemCount);
                    var tempTile = new Kiwi.GameObjects.Sprite(PlayState, PlayState.textures.fruits, tile.my_x * PlayState.tileSize,-(newCount)*PlayState.tileSize);
        ...
        for(var j=this.height-1; j>=0 ;j--){
            ...
            if(!tile.visible){
                ...
                if (lowestVisible == -1) {
                    ...
                    var rand = Math.floor(Math.random() * PlayState.gemCount);
                    var tempTile = new Kiwi.GameObjects.Sprite(PlayState, PlayState.textures.fruits, tile.my_x * PlayState.tileSize, tile.my_y * PlayState.tileSize);
                    tempTile.animation.switchTo(rand);
                    tempTile.my_x = tile.my_x;
                    tempTile.my_y = tile.my_y;
                    tempTile.animating = true;
